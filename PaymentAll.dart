...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const PaymentAll(title: 'Digit Dimes'),
      routes: {
...
class PaymentAll extends StatefulWidget {
  const PaymentAll({super.key, required this.title});
  ...
  @override
  State<PaymentAll> createState() => _PaymentAllState();
}

class _PaymentAllState extends State<PaymentAll> {
...
Future<void> PaymentAll(String u,String p)
async {
  ...
  style: TextStyle(color: Colors.white),
    ...
    ElevatedButton(
      onPressed: () async{
        ...
        print("-----ip");
        print(ip);
        var data = await http.post(Uri.parse("http://"+ip+":5000/and_make_payment_all"),body: {"accno":accno,"prikey":prikey,"amount":amount,'lid':lid});
